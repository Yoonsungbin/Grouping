...
router.post('/SignUp', function(req, res) {
  ...
  var collection = db.get('User');
  collection.insert({"User_Email" : userEmail,"User_Name" : userName,"User_Pass" : userpassword,"Access":'false'}, function (err, doc) {
    if (err) {
      console.log('err');
    }
    else {
...
router.get('/CommunityView',function(req,res){
	...
	Community.findOne({"_id":ObjectID(req.session.CommunityId)},function(err,data){
		if(data == null) {
      console.log('err');
    } else {
     res.send({suc:data});
...
router.post('/up123',function (req, res)  {
  ...
  fs.readFile(req.files.file.path, function(err, data){
   var dirname = path.resolve(".")+'/public/images/';
   console.log(dirname);
